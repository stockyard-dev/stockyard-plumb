...
async function load(){var r=await fetch(A+'/migrations').then(function(r){return r.json()});items=r.migrations||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.version||'').toLowerCase().includes(q)||(i.database||'').toLowerCase().includes(q)||(i.up_sql||'').toLowerCase().includes(q)});
...
if(i.database)h+='<span>'+esc(i.database)+'</span>';
if(i.up_sql)h+='<span>'+esc(i.up_sql)+'</span>';
if(i.down_sql)h+='<span>'+esc(i.down_sql)+'</span>';
if(i.status)h+='<span class="badge">'+esc(i.status)+'</span>';
if(i.applied_at)h+='<span>'+esc(i.applied_at)+'</span>';
if(i.applied_by)h+='<span>'+esc(i.applied_by)+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';

h+='</div>';});document.getElementById('list').innerHTML=h;}
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/migrations/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:"",version:"",database:"",up_sql:"",down_sql:"",status:"",applied_at:"",applied_by:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' PLUMB</h2>';
...
h+='<div class="fr"><label>Database</label><input id="f-database" value="'+esc(i.database)+'"></div>';
h+='<div class="fr"><label>Up S Q L</label><input id="f-up-sql" value="'+esc(i.up_sql)+'"></div>';
h+='<div class="fr"><label>Down S Q L</label><input id="f-down-sql" value="'+esc(i.down_sql)+'"></div>';
h+='<div class="fr"><label>Status</label><input id="f-status" value="'+esc(i.status)+'"></div>';
h+='<div class="fr"><label>Applied At</label><input id="f-applied-at" value="'+esc(i.applied_at)+'"></div>';
h+='<div class="fr"><label>Applied By</label><input id="f-applied-by" value="'+esc(i.applied_by)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),version:document.getElementById('f-version').value.trim(),database:document.getElementById('f-database').value.trim(),up_sql:document.getElementById('f-up-sql').value.trim(),down_sql:document.getElementById('f-down-sql').value.trim(),status:document.getElementById('f-status').value.trim(),applied_at:document.getElementById('f-applied-at').value.trim(),applied_by:document.getElementById('f-applied-by').value.trim()};
if(editId){await fetch(A+'/migrations/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/migrations',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
