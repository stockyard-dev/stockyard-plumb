package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-plumb/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-plumb"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/databases",s.handleListDBs)
    s.mux.HandleFunc("POST /api/databases",s.handleCreateDB)
    s.mux.HandleFunc("DELETE /api/databases/{id}",s.handleDeleteDB)
    s.mux.HandleFunc("GET /api/databases/{id}/migrations",s.handleListMigrations)
    s.mux.HandleFunc("POST /api/databases/{id}/migrations",s.handleRecordMigration)
