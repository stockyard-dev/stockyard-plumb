...
func Open(d string)(*DB,error){if err:=os.MkdirAll(d,0755);err!=nil{return nil,err};db,err:=sql.Open("sqlite",filepath.Join(d,"plumb.db")+"?_journal_mode=WAL&_busy_timeout=5000");if err!=nil{return nil,err}
db.Exec(`CREATE TABLE IF NOT EXISTS migrations(id TEXT PRIMARY KEY,name TEXT NOT NULL,version TEXT DEFAULT '',database TEXT DEFAULT '',up_sql TEXT DEFAULT '',down_sql TEXT DEFAULT '',status TEXT DEFAULT 'pending',applied_at TEXT DEFAULT '',applied_by TEXT DEFAULT '',created_at TEXT DEFAULT(datetime('now')))`)
return &DB{db:db},nil}
